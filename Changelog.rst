...
-  Fix, the ``--lto`` wasn't used for clang yet.
...
-  Avoid decimal and string comparisons for Python versions checks,
   ...
   we now use tuples, otherwise hexacimal values.

-  Stop using child getters and setters, and instead only use subnode
   attributes. This was gradually changed so far, but in this release
   all remaining uses have migrated.
...
-  Change node constructors to not use a decorator to resolve conflicts
   ...
   decorator only made it difficult to read.
...
into the Scons process, avoiding a separate process fork.
...
performance was too slow in some cases. The multiprocess and numpy
plugins were regressed and finally everything ought to be back to
working fine.
